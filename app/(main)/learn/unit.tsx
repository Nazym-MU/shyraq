    ...
    return (
        ...
        <div className="flex items-center flex-col relative">
            {lessons.map((lesson, index) => {
                ...
                return (<LessonButton key={lesson.id} id={lesson.id} index={index} totalCount={lessons.length - 1} current={isCurrent} locked={isLocked} percentage={activeLessonPercentage} />)
            })}
        </div>
        </>
    );
